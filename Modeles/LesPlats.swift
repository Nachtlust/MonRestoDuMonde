...
class LesPlats {
    ...
    func tousLesPlats() -> [Plat] {
        ...
        let tomate = Plat(type: .entree, pays: ITALIEN, desc: "Salade de tomates et Mozzarella", image: #imageLiteral(resourceName: "mozza"))
        lesPlats.append(tomate)
        let pizza = Plat(type: .platPrincipal, pays: ITALIEN, desc: "Pizza Royale (jambon, fromage et champignons)", image: #imageLiteral(resourceName: "pizza"))
        lesPlats.append(pizza)
        let tiramisu = Plat(type: .dessert, pays: ITALIEN, desc: "Le fameux Tiramisu au café, un régal pour les papilles", image: #imageLiteral(resourceName: "tiramisu"))
